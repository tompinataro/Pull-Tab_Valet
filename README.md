...
Pull-Tab Valet is based on the RouteMaster architecture and is configured as its own app identity for mobile and desktop delivery.
...
### Pull-Tab Valet iOS submission setup
```bash
cd mobile
...
Add the App Store Connect app ID in `mobile/eas.json` under `submit.production.ios.ascAppId` before submitting:
```bash
